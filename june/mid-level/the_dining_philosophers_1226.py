...
class DiningPhilosophers:
    ...
    def wantsToEat(self,
        ...
        if philosopher != 0:
            firstFork, secondFork = philosopher, (philosopher - 1) % 5
        else:
            secondFork, firstFork = philosopher, (philosopher - 1) % 5

        with self.locks[firstFork]:
            with self.locks[secondFork]:
                pickLeftFork()
                pickRightFork()
                eat()
                putLeftFork()
...
After an individual philosopher finishes eating, they need to put down both forks so that the forks become available to others. A philosopher
can take the fork on their right or the one on their left as they become available, but cannot start eating before getting both forks.
